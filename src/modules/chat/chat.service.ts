...
import { ErrorCode, ErrorMessage, Status } from 'src/common/enums/responseMessage';
import { ChatRequestHistory, ChatRequestHistoryModel } from './schemas/RequestHistory.schema';
...
export class ChatService {
    constructor(
        ...
        @InjectModel(ChatRequest.name) private chatRequestModel: ChatRequestModel,
        @InjectModel(ChatMessage.name) private chatModel: ChatMessageModel,
        @InjectModel(ChatRequestHistory.name) private chatRequestHistoryModel: ChatRequestHistoryModel
      ) {
    ...
    async createChatThreadRequest(request: any) {
        try {
            const createThreadRequest = await this.chatThreadModel.create(request);

            if( !createThreadRequest ) {
                throw new HttpException({ status: Status.FAILED, errorCode: ErrorCode.NOT_ACCEPTABLE, errorMessage: ErrorMessage.INVALID_USERNAME }, HttpStatus.NOT_FOUND);
            }
            ...
            const createNewThreadRequest = await this.chatRequestModel.create({
                ...
                status: true,
                expiredAt: new Date().setMonth(LiveChatEnum.CHAT_REQUEST_EXPIRATION) 
            });
    ...
    async deleteChatThreadRequest(id: any) {
      try {
          ...
          const requestRecord = await this.chatRequestModel.findOne({ threadId: record._id });
          if( requestRecord ) {

              if( requestRecord.isAccepted ) throw new HttpException({ status: Status.FAILED, errorCode: ErrorCode.NOT_ACCEPTABLE, errorMessage: 'Could not delete this request. It is already accepted.'}, HttpStatus.ACCEPTED);
              ...
              if ( delRequestThread ) {
                ...
                throw new HttpException({ status: Status.FAILED, errorCode: ErrorCode.NOT_ACCEPTABLE, errorMessage: 'Could not delete this request. It is already accepted.'}, HttpStatus.INTERNAL_SERVER_ERROR);
              }
          }
          ...
          throw new HttpException({ status: Status.FAILED, errorCode: HttpStatus.INTERNAL_SERVER_ERROR, errorMessage: ErrorMessage.SOMETHING_WENT_WRONG }, HttpStatus.INTERNAL_SERVER_ERROR);
      }  
    }
